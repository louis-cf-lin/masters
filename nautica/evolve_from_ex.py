...
from controller_from_ex import Controller, ObjTypes
...
## THE POPULATION
pop = [Controller() for _ in range(POP_SIZE)] ## the evolving population (a list of Controllers)
...
def random_light_position(robot) :    
    ...
    # make sure new position is not too close to other objs or the robot
    while is_near_any_objs_or_animat(robot,x,y,2.0 * Obj.RADIUS) :
        x = np.random.rand()*2.0-1.0
        y = np.random.rand()*2.0-1.0
    return x,y
...
def simulate_trial(controller,trial_index,generating_animation=False) :
    ...
    for entity_type in ObjTypes:
        n = {
            ObjTypes.FOOD  : 2,  ## how many of each entity type to create
            ObjTypes.WATER : 2,
            ObjTypes.TRAP  : 2,
        }
        for _ in range(n[entity_type]) :
            x,y = random_light_position(robot)
            entity_light = Obj(x,y,entity_type)
            robot.add_obj(entity_light)

            if entity_type == ObjTypes.FOOD :
                food_entities.append(entity_light)
            if entity_type == ObjTypes.WATER :
                water_entities.append(entity_light)
            if entity_type == ObjTypes.TRAP :
    ...
    for iteration in range(N_STEPS) :
        ...
        for light in robot.objs[ObjTypes.FOOD] :
            if (robot.x - light.x)**2 + (robot.y - light.y)**2 < Obj.RADIUS**2 :
                food_b += 20.0*DT
        ...
        for light in robot.objs[ObjTypes.WATER] :
            if (robot.x - light.x)**2 + (robot.y - light.y)**2 < Obj.RADIUS**2 :
                water_b += 20.0*DT
        ...
        for light in robot.objs[ObjTypes.TRAP] :
            if (robot.x - light.x)**2 + (robot.y - light.y)**2 < Obj.RADIUS**2 :
                food_b -= 50.0*DT
                water_b -= 50.0*DT
                score = 0.0
    ...
    controller.trial_data['uneaten_FOOD_positions']  = [(l.x,l.y) for l in robot.objs[ObjTypes.FOOD]]
    controller.trial_data['uneaten_WATER_positions'] = [(l.x,l.y) for l in robot.objs[ObjTypes.WATER]]
    controller.trial_data['uneaten_TRAP_positions']  = [(l.x,l.y) for l in robot.objs[ObjTypes.TRAP]]
