...
class Link(object):
    """ A single sensor to motor link. A Controller involves several of these."""
    ...
    def set_genome(self,genome) :
        ...
        Uses those values to create a Link. 
...
class Controller(object):
    ...
    def genome_to_links(self) :
        """Translates the genome into a collection of Links that can be used
    ...
    def set_sensor_states(self,entity_type,sensor_values):
        ...
        -- entity_type: the category of entity beind sensed (e.g. ObjTypes.FOOD)

        -- sensor_values: a tuple indicating the excitation of the (left,right) sensor of that type

        a call of set_sensor_states(ObjTypes.WATER,[0,0.9]) is a
    ...
    def procreate_with(self,other_controller,
        ...
        baby = Controller(genome=baby_genome)
        return baby
...
if __name__ == '__main__' :
    l = Link()
    genome = np.random.rand(Link.N_GENES) ## a random genome
